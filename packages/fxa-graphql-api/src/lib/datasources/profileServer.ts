...
import { fxAccountClientToken, configContainerToken } from '../constants';
...
export class ProfileServerSource extends DataSource {
  ...
  private async fetchToken() {
    ...
    const result = await this.authClient.createOAuthToken(this.token, this.oauthClientId, {
  ...
  private async profilePostRequest(path: string, data: object): Promise<superagent.Response> {
  ...
  public async updateDisplayName(name: string) {
    const result = await this.profilePostRequest('/display_name', { displayName: name });
    return result.text === '{}';
  }
